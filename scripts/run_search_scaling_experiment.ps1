Param(
  ...
  [string]$LocustFile = "loadtest/experiments/search_scaling_p95_experiment.py"
)
...
function Run-Locust {
  ...
  locust -f $LocustFile --host=$albUrl --headless `
    ...
    --run-time 15m `
    --html "$ResultsDir/search_p95_scaling_$Desired.html" `
    --csv "$ResultsDir/search_p95_scaling_$Desired"
}

foreach ($desired in 2,5,10) {
  Write-Host "Pinning Search API to desired=$desired (min=max=desired) ..."
